...
public:
  ...
  */
   ...
   */
  enum ms8607_status read_temperature_pressure_humidity(float *t, float *p, float *h);
   ...
   */
  enum ms8607_status get_compensated_humidity(float temperature, float relative_humidity, float *compensated_humidity);
   ...
   * because heater is on
   */
  enum ms8607_status get_dew_point(float temperature, float relative_humidity, float *dew_point);
...
private:
   ...
   *       - ms8607_status_no_i2c_acknowledge : I2C did not acknowledge
   */
  enum ms8607_status psensor_conversion_and_read_adc(uint8_t cmd, uint32_t *adc);
