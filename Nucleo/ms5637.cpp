...
*
*/
MS5637::MS5637(PinName sda, PinName scl) 
{
    ...
    conversion_time = new uint32_t[6];
    
    for(int idx = 0; idx < 6; idx++)
        conversion_time[idx] = tempArray[idx]; // Copies each value into the tones_freq array
...
enum MS5637_status MS5637::write_command(uint8_t cmd) 
{
  ...
  tx[0] = cmd;
  i2c_->write((MS5637_ADDR << 1) & 0xFE, tx, 1);
...
enum MS5637_status MS5637::read_eeprom_coeff(uint8_t command, uint16_t *coeff) 
{
  ...
  i2c_->read((MS5637_ADDR << 1) | 0x01, buffer, 2);
...
enum MS5637_status MS5637::conversion_and_read_adc(uint8_t cmd, uint32_t *adc) 
{
  ...
  wait_ms(conversion_time[(cmd & MS5637_CONVERSION_OSR_MASK) / 2]);
  
  tx[0] = 0x00;
  i2c_->write((MS5637_ADDR << 1) & 0xFE, tx, 1);

  i2c_->read((MS5637_ADDR << 1) | 0x01, buffer, 3);
