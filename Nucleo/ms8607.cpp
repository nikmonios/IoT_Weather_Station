...
{
  //psensor_conversion_time = PSENSOR_CONVERSION_TIME_OSR_256;
 ...
 */
enum MS8607_status MS8607::set_humidity_resolution(enum MS8607_humidity_resolution res)
{
    enum MS8607_status status;
 ...
 *       - MS8607_status_ok
 */
void MS8607::set_humidity_i2c_master_mode(enum MS8607_humidity_i2c_master_mode mode)
{
 ...
 *       - MS8607_status_crc_error : CRC check error
 */
enum MS8607_status MS8607::read_temperature_pressure_humidity(float *t, float *p, float *h)
{
    enum MS8607_status status;
 ...
 *       - MS8607_status_no_i2c_acknowledge : I2C did not acknowledge
 */
enum MS8607_status MS8607::get_heater_status(enum MS8607_heater_status *heater)
{
    enum MS8607_status status;
    uint8_t reg_value;
...
{
    //enum MS8607_status status;
    _i2c.write(HSENSOR_ADDR, (char*)HSENSOR_RESET_COMMAND, 1);
    
    hsensor_conversion_time = HSENSOR_CONVERSION_TIME_12b;
    
    wait_ms(HSENSOR_RESET_TIME);
...
enum MS8607_status MS8607::hsensor_read_user_register(uint8_t *value)
{
    ...
    _i2c.write(HSENSOR_ADDR, (char*)HSENSOR_READ_USER_REG_COMMAND, 1);
    
    
    _i2c.read(HSENSOR_ADDR, (char*)buffer, 1);
...
enum MS8607_status MS8607::hsensor_write_user_register(uint8_t value)
{
    ...
    _i2c.write(HSENSOR_ADDR, (char*)data, 2);
...
enum MS8607_status MS8607::hsensor_humidity_conversion_and_read_adc(uint16_t *adc)
{
    ...
    {
        _i2c.write(HSENSOR_ADDR, (char*)HSENSOR_READ_HUMIDITY_W_HOLD_COMMAND, 1);
    }
    else
    {
        _i2c.write(HSENSOR_ADDR, (char*)HSENSOR_READ_HUMIDITY_WO_HOLD_COMMAND, 1);
        wait_ms(hsensor_conversion_time);
    }
    
    _i2c.read(HSENSOR_ADDR, (char*)buffer, 3);
 ...
 */
enum MS8607_status MS8607::get_compensated_humidity(float temperature, float relative_humidity, float *compensated_humidity)
{
 ...
 *       - MS8607_status_heater_on_error : Cannot compute compensated humidity because heater is on
 */
enum MS8607_status MS8607::get_dew_point(float temperature, float relative_humidity, float *dew_point)
{
...
{
    _i2c.write(PSENSOR_ADDR, (char*)PSENSOR_RESET_COMMAND, 1);
}
...
enum MS8607_status MS8607::psensor_read_eeprom_coeff(uint8_t command, uint16_t *coeff)
{
    ...
    _i2c.write(PSENSOR_ADDR, (char*)command, 1);
    
    //i2c_status = i2c_master_read_packet_wait(&read_transfer);
    _i2c.read(PSENSOR_ADDR, (char*)buffer, 2);
    /*if( i2c_status == STATUS_ERR_OVERFLOW )
        return MS8607_status_no_i2c_acknowledge;
    if( i2c_status != STATUS_OK)
        return MS8607_status_i2c_transfer_error;*/
 ...
 *       - MS8607_status_crc_error : CRC check error on the coefficients
 */
enum MS8607_status MS8607::psensor_read_pressure_and_temperature(float *temperature, float *pressure)
{
    enum MS8607_status status = MS8607_status_ok;
