...
static const int32_t object_temperature_lut[LUT_AMBIENT_SIZE + 1][LUT_ADC_SIZE + 1] PROGMEM = 
{
...
enum tsd305_status tsd305::read_eeprom(void) 
{
  ...
  ASSERT_STATUS(read_eeprom_coeff(0x1a, (uint16_t *)&eeprom_coeff.min_ambient_temperature));
  ASSERT_STATUS(read_eeprom_coeff(0x1b, (uint16_t *)&eeprom_coeff.max_ambient_temperature));
  ...
  ASSERT_STATUS(read_eeprom_coeff(0x2c, (uint16_t *)&eeprom_coeff.adc_calibration_factor));
 ...
 *       - tsd305_status_memory_error : Sensor EEPROM memory error
 */
enum tsd305_status tsd305::conversion_and_read_adcs(uint32_t *adc_object, uint32_t *adc_ambient) 
{
  enum tsd305_status status;
  uint8_t i2c_status;
  uint8_t buffer[7];
 ...
 */
enum tsd305_status
tsd305::read_temperature_and_object_temperature(float *temperature, float *object_temperature) 
{
  enum tsd305_status status = tsd305_status_ok;
  ...
  ASSERT_STATUS(conversion_and_read_adcs((uint32_t *)&adc_object, (uint32_t *)&adc_ambient));

  *temperature = (float)adc_ambient / 16777216.0f * ((float)eeprom_coeff.max_ambient_temperature - (float)eeprom_coeff.min_ambient_temperature) + (float)eeprom_coeff.min_ambient_temperature;

  adc_object = (int64_t)(adc_object - 8388608) * eeprom_coeff.adc_calibration_factor / 1000;

  if (*temperature > get_lut_at(1, 0) && *temperature <= get_lut_at(LUT_AMBIENT_SIZE, 0)) 
  {
    // Look for closest ambient temperature in LUT
    while (*temperature > get_lut_at(adc_ambient_lut_temperature_index, 0) && adc_ambient_lut_temperature_index <= LUT_AMBIENT_SIZE + 1)
      adc_ambient_lut_temperature_index++;

    // Look for closest ADC value in LUT based on min possible ambient
    // temperature
    while (adc_object > get_lut_at(adc_ambient_lut_temperature_index - 1, adc_ambient_lut_adc_index_min) && adc_ambient_lut_adc_index_min <= LUT_ADC_SIZE + 1)
      adc_ambient_lut_adc_index_min++;

    // Look for closest ADC value in LUT based on max possible ambient
    // temperature
    while (adc_object > get_lut_at(adc_ambient_lut_temperature_index, adc_ambient_lut_adc_index_max) && adc_ambient_lut_adc_index_max <= LUT_ADC_SIZE + 1)
    ...
    adc_x1 = get_lut_at(adc_ambient_lut_temperature_index - 1, adc_ambient_lut_adc_index_min - 1);
    adc_x2 = get_lut_at(adc_ambient_lut_temperature_index - 1, adc_ambient_lut_adc_index_min);
    adc_y1 = get_lut_at(adc_ambient_lut_temperature_index, adc_ambient_lut_adc_index_max - 1);
    adc_y2 = get_lut_at(adc_ambient_lut_temperature_index, adc_ambient_lut_adc_index_max);
    ...
    float object_temperature_min = (float)(adc_object - adc_x1) / (float)(adc_x2 - adc_x1) * (float)(obj_min2 - obj_min1) + obj_min1;
    float object_temperature_max = (float)(adc_object - adc_y1) / (float)(adc_y2 - adc_y1) * (float)(obj_max2 - obj_max1) + obj_max1;
